...
use windows_capture::network::{EnhancedNetworkCallback, NetworkConfig, Protocol, FileCallback};
use windows_capture::settings::{
    ColorFormat, CursorCaptureSettings, DirtyRegionSettings, DrawBorderSettings,
    MinimumUpdateIntervalSettings, SecondaryWindowSettings, Settings,
};

/// Handles the trait-based streaming capture events.
struct TraitStreamingCapture {
...
impl GraphicsCaptureApiHandler for TraitStreamingCapture {
    ...
    fn new(ctx: Context<Self::Flags>) -> Result<Self, Self::Error> {
        println!("Trait-based streaming capture started. Press Ctrl+C to stop.");
        ...
        // Create enhanced network callback using trait system
        let callback: Box<dyn windows_capture::encoder::FrameCallback> = match protocol.as_str() {
            "trait-tcp" => {
                ...
                Box::new(EnhancedNetworkCallback::new(config)?)
            }
            "trait-udp" => {
                let config = NetworkConfig {
            ...
            "file" => {
                Box::new(FileCallback::new("trait_streaming_output".to_string()))
            }
            _ => {
    ...
    fn on_frame_arrived(
        ...
        if self.stop_flag.load(Ordering::SeqCst) {
            ...
            println!("Trait streaming stopped.");
        }
    ...
    fn on_closed(&mut self) -> Result<(), Self::Error> {
        println!("\nCapture item closed, stopping trait streaming.");
...
fn main() {
    ...
    println!("Trait streaming protocol: {}", protocol);
    ...
    TraitStreamingCapture::start(settings).expect("Screen capture failed");
}